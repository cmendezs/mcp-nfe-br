...
| `xsd_pl010e_v1.02/` | PL_010e_v.1.02 (released 2026-07-10, official) — bundles NT 2025.002 v1.40, NT 2026.002 v1.0, NT 2026.003 v1.0 | Superseded by `xsd_pl010f_v1.04/` | `[Verified locally — 2026-08-14, BR-NFE-2026-08]`. Delta vs. `PL_010d`: `tpImp` gains enum `6`; `tpEmis`/`indPres` documentation broadened (same enum values); new optional elements `cIndOp` (`ide` group, `[0-9]{6}`) and `ISUFEmit` (`emit` group, `[0-9]{8,9}`); `protNFe/infProt` gains a `maxOccurs="5"` `cMsg`/`xMsg` alert-message sequence (was effectively 0-1); `infNFeSupl/qrCode`'s V3-OFFLINE pattern widens the destinatário-ID segment to alphanumeric-CNPJ-or-CPF; `TTpNFCredito` gains enum `06`. This delta was hand-applied to `src/mcp_nfe_br/schemas/nfe/leiauteNFe_v4.00.xsd` and `leiauteNFe_v4.00_unsigned.xsd`, cited inline per element. |
| `xsd_pl010f_v1.04/` | **PL_010f_v1.04 (released 2026-08-31, official)** — bundles NT 2026.007 v1.00 + NT 2025.002 v1.50 | **Current schema** — regulatory-update, closes `regulatory-update` issue mcp-nfe-br#6 | `[Verified locally — 2026-09-07]`. Diffed the 5 supplied XSDs against `xsd_pl010e_v1.02/`; the only substantive change in `leiauteNFe_v4.00.xsd` is NT 2026.007's IE-optional issuance path: `emit/IE` gains `minOccurs="0"` (was required) — for taxpayers exclusively subject to IBS/CBS; homologação 2026-09-01, **produção 2026-11-03 (still future)**. Cosmetic: `vNFTot`'s declared type renamed `TDec_1302Opc` → `TDec_1302` (the type definitions themselves are byte-identical between packages; `vNFTot` already carries `minOccurs="0"`, so no functional impact). This delta was hand-applied to `src/mcp_nfe_br/schemas/nfe/leiauteNFe_v4.00.xsd` and `leiauteNFe_v4.00_unsigned.xsd`, cited inline per element — same pattern as the 010e delta above. The rest of the `PL_010f` package's diff against `010e` is entirely inside `DFeTiposBasicos_v1.00.xsd`'s `TMonofasia` (IBS/CBS single-phase-taxation) group — a full restructuring (Ad Rem/Ad Valorem choice groups, `gpBioDiferenca`, etc.) that is part of the already-deferred NT 2025.002 v1.40→v1.50 RTC/IBS-CBS modeling effort, **not** re-implemented here; `[NEED: diff NT 2025.002 v1.40 vs v1.50 text]` alongside the existing `xsd_eventos_rtc/` deferral below. **Deliberately not done in this change:** the invoice model's `IE` Pydantic field stays required — NT 2026.007 production enforcement is still future (2026-11-03) and there is no existing IBS/CBS-exclusive-taxpayer classification field to gate the relaxation correctly; tracked as an open (not blocked) roadmap item, activation trigger = that date. |
